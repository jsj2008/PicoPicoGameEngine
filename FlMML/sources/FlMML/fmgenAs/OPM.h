...
private:
	static const int OPM_LFOENTS=512;
	...
	//	static vector<vector<int>*>* amtable;
	//	static vector<vector<int>*>* pmtable;
	...
	static const int sltable[];
	
	static const int slottable[];
	
	Channel4 ch[8];
public:
	OPM() {
		amplevel=FM::FM_VOLENTS;
		chip=new Chip();
		lfo_count_=0;
		lfo_count_prev_=~0;
		BuildLFOTable();
...
protected:
	//  ステータスフラグ解除
	virtual void SetStatus(int bits) {
		if ((status&bits)==0){
			status|=bits;
			Intr(true);
		}
	...
	//  レジスタアレイにデータを設定
	virtual void ResetStatus(int bits) {
		if ((status&bits)!=0){
			status&=~bits;
			if (status==0)
				Intr(false);
...
protected:
	//  パラメータセット
	void SetParameter(int addr,int data) {
...
private:
	...
	//  合成の一部
	void MixSub(int activech,Vector<int>& ibuf) {
	...
	void MixSubL(int activech,Vector<int>& ibuf) {
...
public:
	//  合成 (stereo)
	void Mix(float* buffer,int start,int nsamples) {
		int i;
		...
		if ((activech&0x5555)!=0){
			...
			// Mix
			int a;int c;int r;/*int o;*/int ii;
			int pgex;int pgin;int sino;
			//int al=ch[0].algo_;
			int fb=ch[0].fb;
