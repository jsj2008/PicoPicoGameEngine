...
#if 1
  unsigned short *sndbuf[channels];
  sndbuf[0] = (unsigned short *)samples[0];
  sndbuf[1] = (unsigned short *)samples[1];
	QBSoundLinux* snd = (QBSoundLinux*)QBSound::sharedSound();
	if (!snd->stopWave) {
		snd->playSound();
		if (snd->mBufferSize<count*2*sizeof(short)) {
		  free(snd->mSndBuffer);
		  snd->mBufferSize=count*2*sizeof(short);
      snd->mSndBuffer = calloc(1,snd->mBufferSize);
		}
    snd->fill_sound_buffer(snd->mSndBuffer,count*2*sizeof(short));

    for (int i=0;i<count;i++) {
  		for (chn = 0; chn < channels; chn++) {
     		sndbuf[chn][i] = snd->mSndBuffer[i*2+chn];
      }
	  }
...
static int dosound(int argc, char *argv[])
{
...
int playsound()
{
	QBSoundLinux* snd = new QBSoundLinux(5);
	if (snd) {
		snd->Init();
		snd->Reset();
