...
static int QBSetDefault(const char* key,char* value)
{
	if (WindowsProgramName()) {
		...
		WritePrivateProfileString("PicoPicoGames",key,(const CHAR*)value, szIniFile);
		return 0;
	}
	return -1;
}
	...
	if (value && length != NULL) {
		if (WindowsProgramName()) {
			...
			*value = (char*)malloc(*length);
			...
			while (1) {
				int l = GetPrivateProfileString("PicoPicoGames",key,def,*value,*length,szIniFile);
				if (l < (*length)-1) {
					break;
				}
				free(*value);
...
//	char buf[1024];
//	getApplicationPath(buf,1024);
//	sprintf(fpath,"%s\\Resources\\%s",buf,dbfile);
//	return fpath;
	__tempPath = __watchDataPath;
	__tempPath += dbfile;
	return __tempPath.c_str();
