...
function bgcheck(r)
  ...
  for i,v in pairs(a) do
    v.x=v.x-map[1].x
    if checkbgpos(pppoint(v)) then return true end
  end
  return false
end
...
function checkbgpos(p)
  ...
  return map[1]:check(map[1]:tile(p/32),bgobj)
end
...
--    local mr=mycar:aabb():inset(6)
    if bgcheck(mycar.hitrect+mycar:pos()) then
     return
    end

--    for i,v in pairs(ecar) do
--      if v:isVisible() then
--        if v:aabb():inset(6):intersect(mycar:aabb())
--        then return end
--      end
--    end
