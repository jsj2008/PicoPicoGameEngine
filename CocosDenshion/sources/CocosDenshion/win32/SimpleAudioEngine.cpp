...
const char * _FullPath(const char * szPath)
{
    if (! s_szRootPath[0])
    {
        ...
        strcat(s_szRootPath, "Resources\\");//, sizeof(s_szFullPath));
        strcpy(s_szFullPath, s_szRootPath);//, sizeof(s_szFullPath));
		s_dwRootLen = strlen(s_szRootPath);
//      ++s_dwRootLen;
    }

    if (0 != szPath[0] && ':' != szPath[1])
    {
        strcpy(s_szFullPath + s_dwRootLen, szPath);//, sizeof(s_szFullPath) - s_dwRootLen);
        return s_szFullPath;
    }
    else
    {
